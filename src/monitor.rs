...
impl Inquisidor {
    pub async fn monitor(&self) {
        ...
        let instances = select_instances(&self.ig_pool).await.expect("Failed to select instances.");
        for instance in instances.iter() {
            match instance.instance_status {
                InstanceStatus::New | InstanceStatus::Paused | InstanceStatus::Terminated => continue,
